...
// be used to make precision optimization decisions however, the result of the
...
struct Expression<Operation::Value, FixedFormat<Integer, FractionalBits>> {
    ...
    explicit constexpr Expression(Integer value)
        : value{value} {}
...
struct Expression<Operation::Precision, Precision<FractionalBits>, Expression<Op, Args...>> {
    template <typename T>
    constexpr Expression(Init, T&& value)
        : value{std::forward<T>(value)} {}
    ...
    template <typename TargetFormat>
    constexpr auto Evaluate(TargetFormat) const {
        using IntermediateFormat = FixedFormat<typename TargetFormat::Integer, FractionalBits>;
        return TargetFormat::Convert(value.Evaluate(IntermediateFormat{}));
    }
};
...
private:
    ...
    static constexpr auto Perform(Value<LeftFormat> left, Value<RightFormat> right) {
        if constexpr (LeftFormat::FractionalBits <= RightFormat::FractionalBits) {
            return Value<LeftFormat>{left.value + LeftFormat::Convert(right).value};
        } else {
            return Value<RightFormat>{RightFormat::Convert(left).value + right.value};
        }
    }
};
...
private:
    template <typename LeftFormat, typename RightFormat>
    static constexpr auto Perform(Value<LeftFormat> left, Value<RightFormat> right) {
        // TODO(eieio): Select an intermediate type based on total bits rather than
        // selecting the largest intermediate type.
        using Intermediate = std::conditional_t<sizeof(left) >= sizeof(right),
        ...
        using TargetFormat = FixedFormat<Intermediate, FractionalBits>;

        const Intermediate left_value = left.value;
        const Intermediate right_value = right.value;

        return Value<TargetFormat>{left_value * right_value};
    }
};
...
private:
    ...
    static constexpr auto Perform(TargetFormat, Value<LeftFormat> left, Value<RightFormat> right) {
        ...
        return Value<TargetFormat>{left_value / right.value};
    }
};
...
// definite precision, which could lead to unexpected results.
//
...
// the format with the least precision before comparison.
...
using PrecisionExpression = Expression<Operation::Precision,
                                       Precision<FractionalBits>,
                                       ToExpression<T>>;
