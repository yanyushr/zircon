...
zx_status_t UsbInterface::Create(zx_device_t* parent, UsbComposite* composite,
    ...
    fbl::AllocChecker ac;
    auto interface = fbl::MakeRefCountedChecked<UsbInterface>(&ac, composite->zxdev(), composite, usb);
    if (!ac.check()) {
        return ZX_ERR_NO_MEMORY;
    }

    auto* device_desc = composite->GetDeviceDescriptor();
...
zx_status_t UsbInterface::Create(zx_device_t* parent, UsbComposite* composite,
   ...
   // Interfaces in an IAD interface collection must be contiguous.
    ...
    auto* end = reinterpret_cast<const usb_descriptor_header_t*>(reinterpret_cast<const uint8_t*>(assoc_desc)
                                                                 + desc_length);
...
zx_status_t UsbInterface::UsbConfigureBatchCallback(uint8_t ep_address,
                                                const usb_batch_request_complete_t* complete_cb) {
    return usb_.ConfigureBatchCallback(ep_address, complete_cb);
}
...
size_t UsbInterface::UsbCompositeGetAdditionalDescriptorLength() {
    ...
    auto* end = reinterpret_cast<const usb_descriptor_header_t*>(reinterpret_cast<const uint8_t*>(config)
    ...
    // FIX
    return (uint8_t*)end - (uint8_t*)interface;
}
