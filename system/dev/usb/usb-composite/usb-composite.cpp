...
zx_status_t UsbComposite::AddInterfaces() {
    ...
    while (header < end) {
        if (header->bDescriptorType == USB_DT_INTERFACE_ASSOCIATION) {
            ...
            //FIX
            size_t length = (uint8_t *)next - (uint8_t *)assoc_desc;
            ...
            // FIX
            size_t length = (uint8_t *)next - (uint8_t *)intf_desc;
            if (intf_status == AVAILABLE) {
                ...
                if (interface_statuses_[intf_num] == CLAIMED) {
                    bool removed = RemoveInterfaceById(intf_num);
                    if (!removed) {
                        return ZX_ERR_BAD_STATE;
                    }
                } else {
                    interface_statuses_[intf_num] = CHILD_DEVICE;
                }
            }
            header = next;
        } else {
...
zx_status_t UsbComposite::ClaimInterface(uint8_t interface_id) {
    ...
    if (interface_statuses_[interface_id] == CLAIMED) {
        // The interface has already been claimed by a different interface.
        return ZX_ERR_ALREADY_BOUND;
    } else if (interface_statuses_[interface_id] == CHILD_DEVICE) {
        bool removed = RemoveInterfaceById(interface_id);
        if (!removed) {
            return ZX_ERR_BAD_STATE;
        }
    }
    interface_statuses_[interface_id] = CLAIMED;
...
zx_status_t UsbComposite::GetAdditionalDescriptorList(uint8_t last_interface_id,
    ...
    *out_desc_actual = 0;
    ...
    // FIX
    size_t length = (uint8_t*)end - (uint8_t*)result;
