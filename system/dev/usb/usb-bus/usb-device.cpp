...
void UsbDevice::RequestComplete(usb_request_t* req) {
    ...
    usb_device_req_internal_t* req_int = USB_REQ_TO_DEV_INTERNAL(req, parent_req_size_);
...
zx_status_t UsbDevice::Init() {
    ...
    req_size_ = parent_req_size_ + sizeof(usb_device_req_internal_t);

    list_initialize(&completed_reqs_);
    usb_request_pool_init(&free_reqs_, parent_req_size_ + offsetof(usb_device_req_internal_t, node));
    ...
    if (status != ZX_OK) {
        zxlogf(ERROR, "%s: usb_util_get_descriptor failed\n", __func__);
        return status;
    }
    ...
    // set configuration
    auto* config_desc = reinterpret_cast<usb_configuration_descriptor_t*>(config_descs_[current_config_index_].get());
...
zx_status_t UsbDevice::Reinitialize() {
    ...
    auto* descriptor = reinterpret_cast<usb_configuration_descriptor_t*>(config_descs_[current_config_index_].get());
